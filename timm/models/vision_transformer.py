...
#from .smmix import SummaryMixing
import torch
...
class Attention_split(nn.Module):
    ...
    def __init__(
        ...
        self.scale = self.head_dim ** -0.5
        self.fused_attn = use_fused_attn()
        self.q_lin = nn.ModuleList([nn.Linear(self.head_dim, self.head_dim, bias=qkv_bias) for _ in range(self.num_heads)])
        self.k_lin = nn.ModuleList([nn.Linear(self.head_dim, self.head_dim, bias=qkv_bias) for _ in range(self.num_heads)])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.reshape(B, self.num_heads, -1, N).permute(0,1,3,2)
        q_tensor = []
        for i in range(self.num_heads):
            q_linear = self.q_lin[i]
            output_tensor_q = q_linear(x[:, i, :, :])
            q_tensor.append(output_tensor_q.unsqueeze(1))
        q = torch.cat(q_tensor, dim=1)
        k_tensor = []
        for i in range(self.num_heads):
            k_linear = self.k_lin[i]
            output_tensor_k = k_linear(x[:, i, :, :])
            k_tensor.append(output_tensor_k.unsqueeze(1))
        k = torch.cat(k_tensor, dim=1)
...
class Attention_transpose_all(nn.Module):
    fused_attn: Final[bool]
    ...
    def __init__(
        ...
        self.qkv = nn.Linear(v_dim, v_dim * 3, bias=False)
        #self.v = nn.Linear(v_dim, v_dim, bias=qkv_bias)
        # self.q_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        # self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(v_dim, v_dim, bias=qkv_bias)
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #print("xin",x.shape)
        #eps = 1e-6
        #mean = torch.mean(x, 2, keepdim=False).unsqueeze(-1)
        #std = torch.std(x, 2, keepdim=False).unsqueeze(-1) + eps
        #h = (x - mean) / std
        qkv = self.qkv(x.permute(0,2,1)).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        # q, k = self.q_norm(q), self.k_norm(k)
        #v = self.v(x.permute(0,2,1)).reshape(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        ...
        x = self.proj_drop(x).permute(0,2,1)
        #print("x_out", x.shape)
        return x

class Attention_transpose(nn.Module):
    fused_attn: Final[bool]
...
class Attention_transpose_inference(nn.Module):
    fused_attn: Final[bool]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn = attn.softmax(dim=-1)
        #attn = self.attn_drop(attn)
        attn = torch.matmul(self.v.weight, attn)
        attn = torch.matmul(attn, self.proj.weight)

        std = std.unsqueeze(1).expand(-1, self.num_heads, -1, -1)
        attn = attn + std
        x = x.view(*(B, self.num_heads, -1, self.v_dim))
        x = (x @ attn).transpose(1, 2).contiguous()
        x = x.transpose(1, 2).reshape(B, N, C)+mean
        #print("x_out", h.shape)
        return x
...
class Block_sm(nn.Module):
    def __init__(
        ...
        self.sm = SummaryMixing(enc_dim=dim, nhead=num_heads, summary_out_dim=dim)
        # self.attn = Attention(
        #     dim,
        #     num_heads=num_heads,
        #     qkv_bias=qkv_bias,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = x + self.drop_path1(self.ls1(self.sm(self.norm1(x))))
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        return x

class Block_sm_LLF(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = x + self.drop_path1(self.ls1(self.sm(self.norm1(x))))
        x = x + self.drop_path2(self.ls2(self.mlp2(self.mlp1(self.norm2(x)))))
        return x

class Block_transpose_all(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int,
        ...
        self.attn = Attention_transpose_all(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_norm=qk_norm,
...
class Block_transpose_inference(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int,
        ...
        super().__init__()
        self.attn = Attention_transpose_inference(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_norm=qk_norm,
...
class VisionTransformer_split(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            norm_layer: Optional[LayerType] = None,
            act_layer: Optional[LayerType] = None,
            block_fn: Type[nn.Module] = Block_split,
            mlp_layer: Type[nn.Module] = Mlp,
    ) -> None:
        """
        Args:
...
class VisionTransformer_transpose(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            norm_layer: Optional[LayerType] = None,
            act_layer: Optional[LayerType] = None,
            block_fn: Type[nn.Module] = Block_transpose,
            mlp_layer: Type[nn.Module] = Mlp,
            v_dim = 197,
    ) -> None:
        """
...
class VisionTransformer_transpose_all(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            norm_layer: Optional[LayerType] = None,
            act_layer: Optional[LayerType] = None,
            block_fn: Type[nn.Module] = Block_transpose_all,
            mlp_layer: Type[nn.Module] = Mlp,
    ) -> None:
        """
        Args:
...
class VisionTransformer_transpose_inference(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            norm_layer: Optional[LayerType] = None,
            act_layer: Optional[LayerType] = None,
            block_fn: Type[nn.Module] = Block_transpose_inference,
            mlp_layer: Type[nn.Module] = Mlp,
    ) -> None:
        """
        Args:
...
def _create_vision_transformer_transpose_inference(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer_transpose_inference:
    ...
    return build_model_with_cfg(
        VisionTransformer_transpose_inference,
        variant,
        pretrained,
...
def _create_vision_transformer_transpose_all(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer_transpose_all:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    return build_model_with_cfg(
        VisionTransformer_transpose_all,
        variant,
        pretrained,
        pretrained_filter_fn=_filter_fn,
        pretrained_strict=strict,
...
def _create_vision_transformer_transpose(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer_transpose:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
...
def _create_vision_transformer_split(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer_split:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
...
@register_model
def vit_small_patch16_224_split(pretrained: bool = False, **kwargs) -> VisionTransformer_split:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer_split('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_small_patch16_224_transpose(pretrained: bool = False, **kwargs) -> VisionTransformer_transpose:
    """ ViT-Small_transpose (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer_transpose('vit_small_patch16_224_transpose', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_small_patch16_224_transpose_all(pretrained: bool = False, **kwargs) -> VisionTransformer_transpose_all:
    """ ViT-Small_transpose (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer_transpose_all('vit_small_patch16_224_transpose_all', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_small_patch16_224_transpose_inference(pretrained: bool = False, **kwargs) -> VisionTransformer_transpose_inference:
    """ ViT-Small_transpose (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer_transpose_inference('vit_small_patch16_224_transpose_inference', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
