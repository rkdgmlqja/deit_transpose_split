...
class ParallelLinear(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # elif x.ndim == 4:
        #     B, C, T, F = x.shape
        #     x = x.view(B, C, T, self.n_split, self.split_inp_dim)
...
class SummaryMixing(nn.Module):
    ...
    -------
    >>> x = torch.rand(2,4,8)
    >>> sum = SummaryMixing(enc_dim=8,nhead=8,summary_out_dim=8)
    >>> out = sum(x)
    >>> print(out.shape)
    torch.Size([2, 4, 8])
    """
    ...
    def __init__(
        ...
        enc_dim,
        nhead,
        local_proj_hid_dim: Optional[list] = [64],
        local_proj_out_dim: Optional[int] = 64,
        summary_hid_dim: Optional[list] = [64],
        summary_out_dim: Optional[int] = 64,
    ...
    def forward(self, x, attention_mask=None):
        ...
        if attention_mask is not None:
            attention_mask = torch.logical_not(attention_mask).unsqueeze(-1).float().cuda()
        else:
            attention_mask = torch.ones((x.shape[0], x.shape[1])).unsqueeze(-1).float().cuda()
