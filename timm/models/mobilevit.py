...
from .vision_transformer import Block_sm as SM_TransformerBlock
from .vision_transformer import Block_sm_LLF as SM_LLF_TransformerBlock
from .vision_transformer import Block_LLF as LLF_TransformerBlock
from .vision_transformer import Block_transpose as TransformerBlock_transpose
from .vision_transformer import Block_transpose_inference as TransformerBlock_transpose_inference
__all__ = []
...
def _mobilevit_block_tp_in(d, c, s, transformer_dim, v_dim, transformer_depth, patch_size=4, br=4.0):
    # inverted residual + mobilevit blocks as per MobileViT network
    return (
        _inverted_residual_block(d=d, c=c, s=s, br=br),
        ByoBlockCfg(
            type='mobilevit_tp_in', d=1, c=c, s=1,
            block_kwargs=dict(
                transformer_dim=transformer_dim,
                transformer_v_dim=v_dim,
                transformer_depth=transformer_depth,
                patch_size=patch_size)
        )
    )

def _mobilevit_block_tp(d, c, s, transformer_dim, v_dim, transformer_depth, patch_size=4, br=4.0):
    # inverted residual + mobilevit blocks as per MobileViT network
    return (
        _inverted_residual_block(d=d, c=c, s=s, br=br),
        ByoBlockCfg(
            type='mobilevit_tp', d=1, c=c, s=1,
            block_kwargs=dict(
                transformer_dim=transformer_dim,
                transformer_v_dim=v_dim,
                transformer_depth=transformer_depth,
...
def _mobilevit_block_llf(d, c, s, transformer_dim, transformer_depth, patch_size=4, br=4.0):
    # inverted residual + mobilevit blocks as per MobileViT network
    return (
        _inverted_residual_block(d=d, c=c, s=s, br=br),
        ByoBlockCfg(
            type='mobilevit_llf', d=1, c=c, s=1,
            block_kwargs=dict(
                transformer_dim=transformer_dim,
                transformer_depth=transformer_depth,
                patch_size=patch_size)
...
def _mobilevit_block_sm(d, c, s, transformer_dim, transformer_depth, patch_size=4, br=4.0):
    # inverted residual + mobilevit blocks as per MobileViT network
    return (
        _inverted_residual_block(d=d, c=c, s=s, br=br),
        ByoBlockCfg(
            type='mobilevit_sm', d=1, c=c, s=1,
            block_kwargs=dict(
                transformer_dim=transformer_dim,
                transformer_depth=transformer_depth,
                patch_size=patch_size)
...
def _mobilevit_block_sm_llf(d, c, s, transformer_dim, transformer_depth, patch_size=4, br=4.0):
    # inverted residual + mobilevit blocks as per MobileViT network
    return (
        _inverted_residual_block(d=d, c=c, s=s, br=br),
        ByoBlockCfg(
            type='mobilevit_sm_llf', d=1, c=c, s=1,
            block_kwargs=dict(
                transformer_dim=transformer_dim,
                transformer_depth=transformer_depth,
                patch_size=patch_size)
...
model_cfgs = dict(
    ...
    mobilevit_xxs_tp_in=ByoModelCfg(
        blocks=(
            _inverted_residual_block(d=1, c=16, s=1, br=2.0),
            _inverted_residual_block(d=3, c=24, s=2, br=2.0),
            _mobilevit_block_tp(d=1, c=48, s=2, transformer_dim=64, v_dim=256, transformer_depth=2, patch_size=2, br=2.0),
            _mobilevit_block_tp(d=1, c=64, s=2, transformer_dim=80, v_dim=64, transformer_depth=4, patch_size=2, br=2.0),
            _mobilevit_block_tp(d=1, c=80, s=2, transformer_dim=96, v_dim=16, transformer_depth=3, patch_size=2, br=2.0),
        ),
        stem_chs=16,
        stem_type='3x3',
        stem_pool='',
    ...
    mobilevit_xxs_sm_llf=ByoModelCfg(
        blocks=(
            _inverted_residual_block(d=1, c=16, s=1, br=2.0),
            _inverted_residual_block(d=3, c=24, s=2, br=2.0),
            _mobilevit_block_sm_llf(d=1, c=48, s=2, transformer_dim=64, transformer_depth=2, patch_size=2, br=2.0),
            _mobilevit_block_sm_llf(d=1, c=64, s=2, transformer_dim=80, transformer_depth=4, patch_size=2, br=2.0),
            _mobilevit_block_sm_llf(d=1, c=80, s=2, transformer_dim=96, transformer_depth=3, patch_size=2, br=2.0),
        ),
        stem_chs=16,
        stem_type='3x3',
        stem_pool='',
    ...
    mobilevit_xxs_sm=ByoModelCfg(
        blocks=(
            _inverted_residual_block(d=1, c=16, s=1, br=2.0),
            _inverted_residual_block(d=3, c=24, s=2, br=2.0),
            _mobilevit_block_sm(d=1, c=48, s=2, transformer_dim=64, transformer_depth=2, patch_size=2, br=2.0),
            _mobilevit_block_sm(d=1, c=64, s=2, transformer_dim=80, transformer_depth=4, patch_size=2, br=2.0),
            _mobilevit_block_sm(d=1, c=80, s=2, transformer_dim=96, transformer_depth=3, patch_size=2, br=2.0),
        ),
        stem_chs=16,
        stem_type='3x3',
        stem_pool='',
    ...
    mobilevit_xxs_llf=ByoModelCfg(
        blocks=(
            _inverted_residual_block(d=1, c=16, s=1, br=2.0),
            _inverted_residual_block(d=3, c=24, s=2, br=2.0),
            _mobilevit_block_llf(d=1, c=48, s=2, transformer_dim=64, transformer_depth=2, patch_size=2, br=2.0),
            _mobilevit_block_llf(d=1, c=64, s=2, transformer_dim=80, transformer_depth=4, patch_size=2, br=2.0),
            _mobilevit_block_llf(d=1, c=80, s=2, transformer_dim=96, transformer_depth=3, patch_size=2, br=2.0),
        ),
        stem_chs=16,
        stem_type='3x3',
        stem_pool='',
...
class MobileVitBlock_SM_LLF(nn.Module):
    """ MobileViT block
    ...
    def __init__(
        ...
        super(MobileVitBlock_SM_LLF, self).__init__()
...
@register_notrace_module
class MobileVitBlock_SM(nn.Module):
    """ MobileViT block
        Paper: https://arxiv.org/abs/2110.02178?context=cs.LG
    """
    def __init__(
        ...
        super(MobileVitBlock_SM, self).__init__()
...
@register_notrace_module
class MobileVitBlock_transpose(nn.Module):
    """ MobileViT block
        Paper: https://arxiv.org/abs/2110.02178?context=cs.LG
    """
    def __init__(
            ...
            mlp_ratio: float = 2.0,
            transformer_dim: Optional[int] = None,
            transformer_v_dim: Optional[int] = None,
            transformer_depth: int = 2,
            patch_size: int = 8,
            num_heads: int = 4,
            attn_drop: float = 0.,
        ...
        super(MobileVitBlock_transpose, self).__init__()
...
class MobileVitBlock_transpose_inference(nn.Module):
    """ MobileViT block
        Paper: https://arxiv.org/abs/2110.02178?context=cs.LG
    """
    def __init__(
        ...
        super(MobileVitBlock_transpose_inference, self).__init__()
        ...
        self.transformer = nn.Sequential(*[
            TransformerBlock_transpose_inference(
                transformer_dim,
                mlp_ratio=mlp_ratio,
                num_heads=num_heads,
                qkv_bias=True,
...
@register_notrace_module
class MobileVitBlock_LLF(nn.Module):
    """ MobileViT block
        Paper: https://arxiv.org/abs/2110.02178?context=cs.LG
    """
    def __init__(
        ...
        super(MobileVitBlock_LLF, self).__init__()
...
register_block('mobilevit_tp_in', MobileVitBlock_transpose_inference)
register_block('mobilevit_tp', MobileVitBlock_transpose)
register_block('mobilevit_sm', MobileVitBlock_SM)
register_block('mobilevit_sm_llf', MobileVitBlock_SM_LLF)
register_block('mobilevit', MobileVitBlock)
register_block('mobilevit_llf', MobileVitBlock_LLF)
register_block('mobilevit2', MobileVitV2Block)

def _create_mobilevit_transpose_inference(variant, cfg_variant=None, pretrained=False, **kwargs):
...
def _create_mobilevit_transpose(variant, cfg_variant=None, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ByobNet, variant, pretrained,
        model_cfg=model_cfgs[variant] if not cfg_variant else model_cfgs[cfg_variant],
        feature_cfg=dict(flatten_sequential=True),
...
def _create_mobilevit_llf(variant, cfg_variant=None, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ByobNet, variant, pretrained,
        model_cfg=model_cfgs[variant] if not cfg_variant else model_cfgs[cfg_variant],
        feature_cfg=dict(flatten_sequential=True),
...
def _create_mobilevit_sm(variant, cfg_variant=None, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ByobNet, variant, pretrained,
        model_cfg=model_cfgs[variant] if not cfg_variant else model_cfgs[cfg_variant],
        feature_cfg=dict(flatten_sequential=True),
...
def _create_mobilevit_sm_llf(variant, cfg_variant=None, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ByobNet, variant, pretrained,
        model_cfg=model_cfgs[variant] if not cfg_variant else model_cfgs[cfg_variant],
        feature_cfg=dict(flatten_sequential=True),
...
default_cfgs = generate_default_cfgs({
    'mobilevit_xxs_tp_in.cvnets_in1k': _cfg(hf_hub_id='timm/'),
    'mobilevit_xxs_tp.cvnets_in1k': _cfg(hf_hub_id='timm/'),
    'mobilevit_xxs_sm_llf.cvnets_in1k': _cfg(hf_hub_id='timm/'),
    'mobilevit_xxs_sm.cvnets_in1k': _cfg(hf_hub_id='timm/'),
    'mobilevit_xxs.cvnets_in1k': _cfg(hf_hub_id='timm/'),
    'mobilevit_xxs_llf.cvnets_in1k': _cfg(hf_hub_id='timm/'),
...
@register_model
def mobilevit_xxs_tp_in(pretrained=False, **kwargs) -> ByobNet:
    return _create_mobilevit_transpose_inference('mobilevit_xxs_tp_in', pretrained=pretrained, **kwargs)

@register_model
def mobilevit_xxs_tp(pretrained=False, **kwargs) -> ByobNet:
    return _create_mobilevit_transpose('mobilevit_xxs_tp', pretrained=pretrained, **kwargs)

@register_model
def mobilevit_xxs_sm(pretrained=False, **kwargs) -> ByobNet:
    return _create_mobilevit_sm('mobilevit_xxs_sm', pretrained=pretrained, **kwargs)

@register_model
def mobilevit_xxs_sm_llf(pretrained=False, **kwargs) -> ByobNet:
    return _create_mobilevit_sm_llf('mobilevit_xxs_sm_llf', pretrained=pretrained, **kwargs)

@register_model
def mobilevit_xxs_llf(pretrained=False, **kwargs) -> ByobNet:
    return _create_mobilevit_llf('mobilevit_xxs_llf', pretrained=pretrained, **kwargs)
